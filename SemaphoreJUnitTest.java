...
public class SemaphoreJUnitTest {
  ...
  public void testThatReleaseAllWorksWithThreadsWaiting() throws Exception {
    SemaphoreInterface sem = createSemaphore();
    for(int nbWaitingThread=1; nbWaitingThread < 100; nbWaitingThread++) {
      ...
      //System.err.println("sem.threadBlocked1: "+sem.getThreadBlocked());
      // releasing blocked threads. We use a while loop as we don't
      // know how long the downing threads will take to initialise.
      int totalReleased = 0;
      //System.err.println("nbWaitingThread: "+nbWaitingThread);
      while(totalReleased<nbWaitingThread) {
        totalReleased += sem.releaseAll();
    	//System.err.println("totalReleased: "+totalReleased);
    	//Thread.sleep(1000);
        Thread.yield();
      } // EndWhile
      
      //System.err.println("sem.threadBlocked2: "+sem.getThreadBlocked());
